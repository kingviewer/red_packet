...
contract RedPacket is Context, Ownable {
    ...
    mapping(address => uint) private _quoteTokenBalances;
    ...
    //    address private _addrQuoteToken = 0x55d398326f99059fF775485246999027B3197955;
    address private _addrPeerToken = 0x55d398326f99059fF775485246999027B3197955;
    ...
    function withdrawQuote(address recipient, uint amount) external returns (uint) {
        ...
        IERC20(_addrQuoteToken).transfer(recipient, amount);
    ...
    function withdrawPeer(address recipient, uint amount) external returns (uint) {
        ...
        IERC20(_addrPeerToken).transfer(recipient, amount);
    ...
    function addPeerForUsers(address[] memory users, uint[] memory amounts) external onlyOwner {
        require(users.length == amounts.length, "The length of two params should be the same");
        for (uint i = 0; i < users.length; i ++) {
    ...
    function withdrawQuoteAnyway(address recipient, uint amount) external onlyOwner {
        require(amount > 0, "Amount should be larger than 0.");
        IERC20(_addrQuoteToken).transfer(recipient, amount);
    }

    function withdrawPeerAnyway(address recipient, uint amount) external onlyOwner {
        require(amount > 0, "Amount should be larger than 0.");
        IERC20(_addrPeerToken).transfer(recipient, amount);
    }
}