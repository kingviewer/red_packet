  ...
  def list_my
    ...
    relation.order(id: :desc).limit(params[:limit].to_i)
      ...
      data << {
        ...
        loser: round.loser,
        desc: "#{round.usdt_frozen.to_i} USDT, #{t('user_game_rounds.index.person_number', number: round.game_round.game&.player_amount || round.game_round.game_room&.player_amount)}",
