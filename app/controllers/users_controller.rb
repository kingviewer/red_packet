class UsersController < BaseUserController
  ...
  def my_info
    ...
    success(
      ...
      chain_cigar_display: LZUtils.format_money(cur_user.chain_cigar_available),
      candy_available: cur_user.candy_available,
      candy_available_display: LZUtils.format_money(cur_user.candy_available),
      candy_frozen: cur_user.candy_frozen,
      candy_frozen_display: LZUtils.format_coin(cur_user.candy_frozen),
      candy: cur_user.candy,
      candy_display: LZUtils.format_money(cur_user.candy),
  ...
  def my_friends
    data = []
    cur_user.children.order(id: :desc).limit(params[:limit].to_i).offset((params[:page].to_i - 1) * params[:limit].to_i).each do |child|
      data << {
        id: child.id,
