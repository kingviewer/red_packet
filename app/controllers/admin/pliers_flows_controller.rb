class Admin::PliersFlowsController < Admin::BaseController
  ...
  def list_relation
    relation = PliersFlow.includes(:user).joins(:user)
    ...
    relation = relation.where(['pliers_flows.created_at >= ?', "#{params[:start_at].strip} 00:00:00.000000"]) unless params[:start_at].blank?
    relation = relation.where(['pliers_flows.created_at <= ?', "#{params[:end_at].strip} 23:59:59.999999"]) unless params[:end_at].blank?
    relation
  end
end
